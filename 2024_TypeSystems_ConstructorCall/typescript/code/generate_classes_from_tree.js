...
import { do_random_array_sort, random_array_element, random_integer_up_to_excluding } from "../../../N-of-1-Experimentation/modules/Experimentation/Experimentation.js";
import { Nouns } from "../../../N-of-1-Experimentation/modules/Words/Nouns.js";
export function generate_classes_from_tree(tree, number_of_classes) {
    ...
    function get_random_class_name() {
        Math.random() * (CLASS_NAMES.length);
        return CLASS_NAMES.shift();
    }
    function get_random_parameter_name() {
        Math.random() * (PARAMETER_NAMES.length);
        return PARAMETER_NAMES.shift();
    }
    function get_random_function_name() { return FUNCTION_NAMES.shift(); }
    ...
    tree.postorder((t) => {
        ...
        let class_definition = new Class_Definition(t.content, child_param_names, child_type_names, get_random_function_name());
    ...
    for (let i = classes.number_of_classes(); i < number_of_classes; i++) {
        ...
        for (let c = 0; c < random_param_number; c++) {
            ...
            child_param_types.push(random_array_element(classes.class_definitions).class_name);
        }
        let class_definition = new Class_Definition(get_random_class_name(), child_param_names, child_param_types, get_random_function_name());
        classes.class_definitions.push(class_definition);
    }
    classes.sort_class_definitions();
    return classes;
}
//# sourceMappingURL=generate_classes_from_tree.js.map