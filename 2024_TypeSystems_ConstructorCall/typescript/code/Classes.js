export class Classes {
    constructor(class_definitions) {
        this.class_definitions = class_definitions;
    }
    type_check(constructor_call) {
        let this_class = this.get_class_named(constructor_call.class_name);
    ...
    html_table_string(num_columns, with_types) {
    ...
    sort_class_definitions() {
        this.class_definitions.sort((c1, c2) => {
            if (c1.class_name === "Target")
                return 1;
            if (c2.class_name === "Target")
                return -1;
